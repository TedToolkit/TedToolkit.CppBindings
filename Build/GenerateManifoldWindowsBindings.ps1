...
function Get-ManifoldOutputFiles {
    param([Parameter(Mandatory = $true)] [string] $Root, [Parameter(Mandatory = $true)] [string] $Configuration)
    ...
    foreach ($relativePath in @(
            ...
            "native-build/$Configuration/ted_toolkit_cpp_bindings_manifold.dll")) {
...
function Write-ManifoldOutputManifest {
    param(
        [Parameter(Mandatory = $true)] [string] $Root,
        [Parameter(Mandatory = $true)] [string] $Configuration,
        [Parameter(Mandatory = $true)] [string] $Manifest)

    $entries = @(Get-ManifoldOutputFiles -Root $Root -Configuration $Configuration | ForEach-Object {
        [ordered]@{
    ...
    if ($entries.Count -eq 0) { throw 'Cannot record an empty Manifold output manifest.' }
    [ordered]@{ Files = $entries } | ConvertTo-Json -Depth 5 |
        Set-Content -LiteralPath $Manifest -Encoding utf8
}

function Test-ManifoldOutputManifest {
    param(
        [Parameter(Mandatory = $true)] [string] $Root,
        [Parameter(Mandatory = $true)] [string] $Configuration,
        [Parameter(Mandatory = $true)] [string] $Manifest)
    ...
    try {
        $recorded = @(Get-Content -LiteralPath $Manifest -Raw | ConvertFrom-Json |
            Select-Object -ExpandProperty Files)
        $actual = @(Get-ManifoldOutputFiles -Root $Root -Configuration $Configuration)
...
$mutex = [Threading.Mutex]::new($false, "Local\TedToolkit.CppBindings.Manifold.Windows.$mutexHash")
try {
    try { $mutex.WaitOne() | Out-Null } catch [Threading.AbandonedMutexException] { }

    $nativeLibrary = Join-Path $generated "native-build\$Configuration\ted_toolkit_cpp_bindings_manifold.dll"
    ...
    $inputs = @(
        (Get-ChildItem -LiteralPath (Join-Path $repository 'src/providers/manifold') -Recurse -File |
    ...
    $noticesComplete = @('Manifold.txt', 'Clipper2.txt', 'TBB.txt').Where({
    ...
    $outputsComplete = Test-ManifoldOutputManifest -Root $generated -Configuration $Configuration `
        -Manifest $outputManifest
    if ((Test-Path -LiteralPath $stamp -PathType Leaf) `
        -and (Get-Content -LiteralPath $stamp -Raw) -ceq $fingerprint `
        -and (Test-Path -LiteralPath (Join-Path $generated 'csharp/Manifold.Bindings.g.cs') -PathType Leaf) `
        -and $dependenciesComplete -and $noticesComplete -and $outputsComplete) {
        Write-Output 'Manifold Windows bindings are up to date.'
        exit 0
    }
    ...
    Assert-NativeBuildDiskBoundary -Path $generated -Phase 'Manifold generation' `
        -ScratchRoot $scratchRoot | Out-Null
    & dotnet $GeneratorHost --output-root $generated
    if ($LASTEXITCODE -ne 0) { throw "The Manifold generator exited with code $LASTEXITCODE." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Manifold native configuration failed with exit code $LASTEXITCODE." }

    Assert-NativeBuildDiskBoundary -Path $generated -Phase 'Manifold native compilation' `
        -ScratchRoot $scratchRoot | Out-Null
    & cmake --build (Join-Path $generated 'native-build') --config $Configuration --parallel 1
    if ($LASTEXITCODE -ne 0) { throw "Manifold native build failed with exit code $LASTEXITCODE." }
    if (-not (Test-Path -LiteralPath $nativeLibrary -PathType Leaf)) {
        throw "The Manifold native library was not produced at '$nativeLibrary'."
    }

    $profile = Get-Content -LiteralPath (Join-Path $generated 'csharp/profile-manifest.json') -Raw |
        ConvertFrom-Json
    $compilerParts = (Get-Content -LiteralPath (Join-Path $generated 'native-build/compiler-identity.txt') -Raw).
        Trim().Split('|', 3)
    if ($compilerParts.Count -ne 3 -or $compilerParts[0] -cne 'MSVC' `
        -or $compilerParts[1] -cne "$($profile.Msvc).0") {
    ...
    if ($cmakeVersionLine -notmatch '^cmake version (.+)$' -or $Matches[1] -cne $profile.CMake) {
    ...
    ([ordered]@{
        'Manifold.txt' = Join-Path $vcpkg 'installed/x64-windows/share/manifold/copyright'
        'Clipper2.txt' = Join-Path $vcpkg 'installed/x64-windows/share/clipper2/copyright'
        'TBB.txt' = Join-Path $vcpkg 'installed/x64-windows/share/tbb/copyright'
    }).GetEnumerator() | ForEach-Object {
        if (-not (Test-Path -LiteralPath $_.Value -PathType Leaf)) {
            throw "Required notice input '$($_.Value)' is missing."
        }
        Copy-Item -LiteralPath $_.Value -Destination (Join-Path $notices $_.Key)
    }

    Write-ManifoldOutputManifest -Root $generated -Configuration $Configuration -Manifest $outputManifest
