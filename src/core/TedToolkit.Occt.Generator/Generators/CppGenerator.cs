...
internal sealed class CppGenerator(RecordModel record) : IGenerator
{
    ...
    public Task<string> GenerateAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _ = builder.Append("#include \"NativeError.h\"\n#include <Standard_Failure.hxx>\n#include <exception>\n");
        }
    ...
    private static IEnumerable<string> GetRequiredHeaders(RecordModel recordModel)
    {
        ...
        return headers.Where(static header => header is not "Standard.hxx" and not "Standard_Handle.hxx");
    }
    ...
    private void AppendMethod(StringBuilder builder, MethodModel method, string exportName)
    {
        switch (method.Type)
        {
            ...
            case MethodModelType.DELETE:
                ...
                if (record.ObjectKind is NativeObjectKind.Handle)
                {
                    _ = builder.Append("    if (self != nullptr && self->DecrementRefCounter() == 0)\n")
                        .Append("    {\n        self->Delete();\n    }\n");
                }
                else
                {
            ...
            case MethodModelType.HANDLE_RELEASE:
                _ = builder.Append("extern \"C\" void ").Append(exportName).Append('(')
                    .Append(record.Type.CppTypeName).Append("* self) noexcept\n{\n")
                    .Append("    if (self != nullptr && self->DecrementRefCounter() == 0)\n")
                    .Append("    {\n        self->Delete();\n    }\n}\n\n");
                return;
    ...
    private void AppendConstructor(StringBuilder builder, MethodModel method, string exportName)
    {
        ...
        if (record.ObjectKind is NativeObjectKind.Handle)
        {
            ...
            var allocator = record.UsesAllocatorPlacementNew
                ? method.Parameters.FirstOrDefault(static parameter =>
                    parameter.Type.IsOcctHandle
                    && parameter.Type.OcctHandleElementCppType?.Contains("Allocator", StringComparison.Ordinal)
                    is true)
                : null;
    ...
    {
        var returnsHandleValue = method.ReturnType.IsOcctHandle
        ...
        {
            _ = builder.Append(method.ReturnType.OcctHandleElementCppType).Append("* ")
    ...
    private static void AppendErrorParameter(StringBuilder builder, MethodModel method, bool hasPrevious)
    {
        if (method.NoExceptions)
        {
            return;
        ...
        _ = builder.Append("NativeError* __error");
    }
    ...
    private static void AppendTryEnd(StringBuilder builder, MethodModel method, string failureReturn)
    {
        if (method.NoExceptions)
        {
            return;
        ...
        _ = builder.Append("    }\n    catch (const std::bad_alloc& exception)\n    {\n")
            .Append("        NativeError_Set(__error, 6, \"std::bad_alloc\", exception.what());\n        ")
            .Append(failureReturn)
            .Append("\n    }\n    catch (const Standard_Failure& exception)\n    {\n")
            .Append("        NativeError_Set(__error, 8, \"Standard_Failure\", exception.what());\n        ")
            .Append(failureReturn)
            .Append("\n    }\n    catch (const std::exception& exception)\n    {\n")
            .Append("        NativeError_Set(__error, 9, \"std::exception\", exception.what());\n        ")
            .Append(failureReturn)
            .Append("\n    }\n    catch (...)\n    {\n")
            .Append("        NativeError_Set(__error, 255, nullptr, nullptr);\n        ")
            .Append(failureReturn)
            .Append("\n    }\n");
    }
