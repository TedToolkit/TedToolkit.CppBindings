...
internal static class Helpers
{
    ...
    /// <param name="maximumLength">The physical filename budget, including the stable hash suffix.</param>
    /// <returns>The original name, or a readable prefix followed by a stable content hash.</returns>
    public static string ToGeneratedFileStem(this string name, int maximumLength = MaximumGeneratedFileStemLength)
    {
        ArgumentNullException.ThrowIfNull(name);
        ArgumentOutOfRangeException.ThrowIfLessThan(maximumLength, 18);
        ...
        return string.Concat(
            name.AsSpan(0, maximumLength - hash.Length - 1),
            "_",
            hash);
    }
