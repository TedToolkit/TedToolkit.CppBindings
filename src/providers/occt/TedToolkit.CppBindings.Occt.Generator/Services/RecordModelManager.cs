    ...
    IVcpkgEnvironment vcpkgEnvironment) : IRecordModelManager
{
    ...
    public RecordModel Add(CXXRecordDecl record)
    {
        ...
        result.Bases = isOcctType
            ? record.Bases
                ...
                .ToArray()
            : [];
        result.UsesIntrusiveReferenceCounting = DerivesFromStandardTransient(record, []);
    ...
    private void FinalizeManagedTemplateReferences()
    {
        ...
        {
            if (record.FieldModels.Any(static field => field.UsesIntrusiveHandleReferenceStorage)
    ...
    private static string GetTemplatePhysicalShape(RecordModel record)
    {
        return string.Join(
            "\n",
            record.ObjectKind,
            record.UsesIntrusiveReferenceCounting,
        ...
        CommentProjection methodCommentProjection)
    {
        ...
        var type = ToModel(paramDel.Type);
        ...
        return new()
        {
            ...
            Type = type,
            IsPlacementAllocator = type.IsIntrusiveHandle
                && type.IntrusiveHandleElementCppType.Contains("Allocator", StringComparison.Ordinal),
