// -----------------------------------------------------------------------
// <copyright file="OcctCompilerProbeModule.cs" company="TedToolkit">
...
/// </summary>
[DependsOn<OcctParseModule>]
public sealed class OcctCompilerProbeModule : Module<bool>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="OcctCompilerProbeModule"/> class.
    /// </summary>
    ...
    internal OcctCompilerProbeModule(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var temporaryDirectory = Directory.CreateTempSubdirectory("TedToolkit.CppBindings.Occt.Probe.");
        try
        {
    ...
    internal static void CompleteRecords(RecordModel[] records, string output)
    {
        ...
        foreach (var line in lines)
        {
            ...
            else if (record.ObjectKind is NativeObjectKind.IntrusiveHandle
                     && isDestructible
    ...
    private static HashSet<string> LoadUnsupportedNativeExports()
    {
        const string resourceName =
            "TedToolkit.CppBindings.Occt.Generator.Resources.UnsupportedNativeExports.txt";
        using var stream = typeof(OcctCompilerProbeModule).Assembly.GetManifestResourceStream(resourceName)
                           ?? throw new InvalidOperationException(
    ...
    private static void AddHandleReleaseOperations(RecordModel[] records)
    {
        var handleTargets = records
            ...
            .Where(static type => type.IsIntrusiveHandle)
            .Select(static type => type.IntrusiveHandleElementCppType)
            .ToHashSet(StringComparer.Ordinal);

        foreach (var record in records.Where(record =>
                     record.ObjectKind is NativeObjectKind.IntrusiveHandle
                     && handleTargets.Contains(record.Type.CppTypeName)
                     && !record.MethodModels.Any(static method =>
                         method.Type is MethodModelType.INTRUSIVE_RELEASE
                             ...
                             && method.NativeExportName.EndsWith("_Release", StringComparison.Ordinal))))
        {
            ...
            [
                .. record.MethodModels,
                CreateImplicitDestructor(MethodModelType.INTRUSIVE_RELEASE),
            ];
            NativeExportNameBuilder.Assign(record);
        }
    }
    ...
    {
        if (record.UsesIntrusiveReferenceCounting)
        {
            return NativeObjectKind.IntrusiveHandle;
        }
