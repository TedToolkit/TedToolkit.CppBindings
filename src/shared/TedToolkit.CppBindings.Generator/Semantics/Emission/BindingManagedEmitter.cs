    ...
    bool generateRepresentation = true) : IBindingSourceEmitter
{
    ...
    private string GenerateExtensions()
    {
        ...
        foreach (var method in methods)
        {
            ...
            if (!method.IsStatic && recordDecl.ObjectKind is NativeObjectKind.IntrusiveHandle)
            {
    ...
    private void AppendConstructorExtension(StringBuilder builder, MethodModel method, string recordName)
    {
        ...
        if (recordDecl.ObjectKind is not NativeObjectKind.IntrusiveHandle)
        {
        ...
        {
            NativeObjectKind.IntrusiveHandle => $"{emissionProfile.ManagedOwningIntrusiveHandle}<{recordName}>",
        ...
        IReadOnlyList<string> parameterOwners)
    {
        ...
        _ = builder.Append("                return new ").Append(emissionProfile.ManagedOwningIntrusiveHandle).Append('<')
        ...
        bool borrowedHandleReceiver)
    {
        ...
        else if (method.ReturnType.IsIntrusiveHandle)
        {
        ...
        string indentation)
    {
        ...
        if (!method.IsStatic && !borrowedHandleReceiver
            && recordDecl.ObjectKind is NativeObjectKind.IntrusiveHandle or NativeObjectKind.Owned)
        {
        ...
        bool borrowedHandleReceiver)
    {
        ...
        AppendMethodDeclarationStart(
            builder,
            $"{emissionProfile.ManagedOwningIntrusiveHandle}<{elementType}>?",
            method);
        _ = builder.AppendJoin(", ", publicParameters).Append(')');
        AppendReceiverConstraint(builder, method);
        _ = builder.Append("\n        {\n");
        ...
        _ = builder.Append("                if (__result == null)\n                {\n")
            .Append("                    return null;\n                }\n\n")
            .Append("                return new ").Append(emissionProfile.ManagedOwningIntrusiveHandle).Append('<')
            ...
            .Append(elementType).Append("*, void>)").Append(GetNativeFunctionExpression(
                NativeExportNameBuilder.GetIntrusiveReleaseName(method.ReturnType.IntrusiveHandleElementCppType)))
            .Append(");\n            }\n        }\n\n");
    }
    ...
    {
        _ = builder.Append(indentation).Append(emissionProfile.ManagedNativeErrorProjection).Append(".ThrowIfFailed(\n")
            ...
            .Append(GetNativeFunctionExpression(emissionProfile.NativeErrorClearExport)).Append(");\n");
    }
        ...
        List<(string Type, string Name, string Expression)> pins)
    {
        ...
        var publicReceiver = borrowedHandleReceiver
            ...
            {
                NativeObjectKind.IntrusiveHandle => $"this {emissionProfile.ManagedOwningIntrusiveHandle}<{receiverType}> self",
        ...
        pins.Add((receiverType, "selfPointer",
            borrowedHandleReceiver || recordDecl.ObjectKind is NativeObjectKind.IntrusiveHandle or NativeObjectKind.Owned
                ? "&self.Value"
                : "&self"));
    }
    ...
    {
        return $"this in {emissionProfile.ManagedBorrowedIntrusiveHandle}<{receiverType}> self";
    }
    ...
    private void AppendReceiverConstraint(StringBuilder builder, MethodModel method)
    {
        ...
        {
            _ = builder.Append(", ").Append(emissionProfile.ManagedIntrusiveRootInterface);
        }
        else if (recordDecl.ObjectKind is NativeObjectKind.Owned)
        {
        ...
        List<string> parameterOwners)
    {
            ...
            && parameterRecord.ObjectKind is NativeObjectKind.Owned or NativeObjectKind.IntrusiveHandle)
        {
            ...
            var ownerName = parameterRecord.ObjectKind is NativeObjectKind.Owned
                ...
                : emissionProfile.ManagedOwningIntrusiveHandle;
    ...
    private string GetNativeFunctionExpression(string exportName)
    {
        ...
        return $"global::{emissionProfile.CSharpNamespace}.NativeApi.GetFunction({index})";
    }
