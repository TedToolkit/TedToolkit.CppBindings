...
{
    private const string HandleMetadataName = "TedToolkit.Occt.Handle`1";

    private const string OwnedMetadataName = "TedToolkit.Occt.Owned`1";

    /// <summary>
    /// Identifies a supported unsafe use of a non-owning owner Value reference.
    /// </summary>
    public const string DiagnosticId = "TTOCCT002";
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        {
            var handle = compilationContext.Compilation.GetTypeByMetadataName(HandleMetadataName);
            var owned = compilationContext.Compilation.GetTypeByMetadataName(OwnedMetadataName);
            if (handle is null && owned is null)
            {
                return;
            }

            compilationContext.RegisterSyntaxNodeAction(
                syntaxContext => AnalyzeValueAccess(in syntaxContext, handle, owned),
        ...
        INamedTypeSymbol? owned)
    {
        ...
        var property = context.SemanticModel.GetSymbolInfo(memberAccess, context.CancellationToken).Symbol
            as IPropertySymbol;
        if (!IsOwnerValue(property, handle, owned))
        {
            return;
        }
        ...
        INamedTypeSymbol? owned)
    {
        ...
        var ownerType = property.ContainingType.OriginalDefinition;
        return SymbolEqualityComparer.Default.Equals(ownerType, handle)
               || SymbolEqualityComparer.Default.Equals(ownerType, owned);
    }
