...
/// <summary>
/// <see cref="OcctParseModule"/> execution behavior.
/// </summary>
internal sealed class ExecuteAsyncTests
{
    ...
    public async Task Should_parse_only_the_selected_real_occt_header_Async()
    {
        ...
        try
        {
            var declarations = new OcctDeclarationOptions[] { new(Target), };
            ...
            var module = new OcctParseModule(recordManager, options, tripletResolver, environment);
    ...
    private static Task<bool> ExecuteAsync(OcctParseModule module, IModuleContext context)
    {
        var method = typeof(OcctParseModule).GetMethod("ExecuteAsync", BindingFlags.Instance | BindingFlags.NonPublic);
        if (method is null)
        {
            throw new InvalidOperationException("OcctParseModule.ExecuteAsync was not found.");
        }
    ...
    private sealed class ParseFixture : IDisposable
    {
        ...
        public OcctParseModule Module { get; }
            ...
            IReadOnlyList<OcctDeclarationOptions> declarations)
        {
            ...
            var recordManager = new CapturingRecordModelManager();
            var module = new OcctParseModule(recordManager, options, new FixtureTripletResolver(), environment);
