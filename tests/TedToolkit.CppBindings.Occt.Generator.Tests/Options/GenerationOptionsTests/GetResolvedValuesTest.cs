...
namespace TedToolkit.CppBindings.Occt.Generator.Tests.Options.GenerationOptionsTests;

/// <summary>
/// Verifies <see cref="OcctGenerationOptions"/> resolves configured values correctly.
/// </summary>
internal sealed class GetResolvedValuesTest
{
    ...
    public async Task Should_return_clean_native_library_basename_when_name_is_not_configured_Async()
    {
        ...
        var baseName = options.NativeLibraryBaseName;

        await Assert.That(baseName).IsEqualTo("ted_toolkit_occt");
    }

    private static OcctGenerationOptions CreateOptions()
    {
        return new()
        {
            DeclOptions = [],
