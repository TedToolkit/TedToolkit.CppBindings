...
namespace TedToolkit.CppBindings.Occt.Generator.Tests;
...
internal sealed class HelpersTest
{
    ...
    public async Task Should_shorten_only_long_generated_file_names_Async()
    {
        ...
        var nativeStem = longName.ToGeneratedFileStem(80);
        await Assert.That(nativeStem.Length).IsEqualTo(80);
        await Assert.That(nativeStem).IsEqualTo(longName.ToGeneratedFileStem(80));
        await Assert.That(nativeStem).IsNotEqualTo((longName + "B").ToGeneratedFileStem(80));
    }
