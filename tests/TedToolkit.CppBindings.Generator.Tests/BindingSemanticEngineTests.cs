...
internal sealed class BindingSemanticEngineTests
{
    ...
    /// <summary>
    /// Verifies declarations in one managed source group share one source file and namespace.
    /// </summary>
    /// <returns>A task that completes when the grouped source has been parsed.</returns>
    [Test]
    public async Task Should_compose_a_managed_source_group_as_one_source_file_Async()
    {
        var engine = CreateEngine([new TypeRule(true, "Projected"),]);
        var firstSource = CreateDeclaration(
            "fixture::first",
            "First",
            isRoot: true,
            [],
            "FirstExport");
        var secondSource = CreateDeclaration(
            "fixture::second",
            "Second",
            isRoot: true,
            [],
        ...
        {
            new BindingDeclaration(firstSource.Record, "fixture::family", isRoot: true, []),
            new BindingDeclaration(secondSource.Record, "fixture::family", isRoot: true, []),
        };
        var providerModel = new BindingProviderModel(
            declarations,
            [],
        ...
        var plan = engine.CreatePlan(providerModel);
        var source = await RenderAsync(plan.CSharpSources.Single(static candidate =>
            candidate.RelativePath == "fixture_family.g.cs")).ConfigureAwait(false);
        ...
        await Assert.That(source.Split("<auto-generated>", StringSplitOptions.None).Length - 1).IsEqualTo(1);
        await Assert.That(source.Split("namespace Fixture.Bindings", StringSplitOptions.None).Length - 1).IsEqualTo(1);
        await Assert.That(source).Contains("public unsafe struct First");
        await Assert.That(source).Contains("public static unsafe class SecondExtensions");
