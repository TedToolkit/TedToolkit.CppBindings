# Binding analyzer boundary

- Status: Active
- Owner: TedToolkit maintainers
- Scope and system boundary: Compile-time tooling that distinguishes generated binding
  implementation code from handwritten consumers of `TedToolkit.CppBindings.Runtime` and generated OCCT
  packages.
...
Generator model validation and generated API shape
             |
             v
Binding analyzer diagnostics at consumer compilation
             |
             v
...
| `TedToolkit.CppBindings.Occt.SourceGenerators` | Generator development/build | Generate the OCCT Generator's configured header input | Consumer policy, Runtime ownership, or native declaration validation |
...
| `TedToolkit.CppBindings.Analyzers` | Consumer compilations that directly reference the analyzer package | Report handwritten use of explicitly marked generated-only Runtime hooks and supported suspicious uses of non-owning owner `Value` references | Runtime's own compilation, OCCT declaration knowledge, complete alias or concurrency proof, security, or runtime correctness |
| `TedToolkit.CppBindings.Runtime` | Managed execution | Validate generic constraints, pointer and function inputs available to each Runtime API, owner state, and disposed state when `Value` is obtained | Authenticating a function pointer's native-module origin, exact-match module initialization, process-lifetime module loading, revoking an already-returned reference, preventing concurrent disposal, generated declaration catalogs, or analyzer implementation |

The consumer analyzer is a standalone Roslyn package, not an extension of the OCCT-header source
generator. Runtime declares the sealed, declaration-agnostic public
...
`TTCB001`, reports an error when handwritten code operationally references a marked constructor,
...
A separate declaration-agnostic lifetime rule, `TTCB002`, reports an error for supported
...
`InternalsVisibleTo`, or special treatment of `TedToolkit.CppBindings.Occt.Windows`. It exempts a call site from
...
Runtime contracts. `TTCB002` diagnoses supported unsafe usage shapes around `Value`; it does not
...
The analyzer is packaged independently as `TedToolkit.CppBindings.Analyzers`. Consumers reference
it directly with `PrivateAssets="all"`; it neither enters Runtime output nor flows implicitly through
Runtime or generated binding packages. Runtime validation remains mandatory because
...
- `TTCB001` is an error by default and states that the referenced API is reserved for generated C++
  binding code. Its documentation provides the supported generated factory or operation when
...
- `TTCB002` is an error by default, identifies the supported unsafe `Value` lifetime pattern, and
...
- Do not require a managed borrowed-reference wrapper, owner-retaining facade, lease, or
  ...
  with the caller.
- Generator structural and integration proof, not binding analyzer diagnostics, verifies that each
...
- Every generated-only Runtime entry point retains complete runtime validation of the inputs and
  state actually available at invocation time. Generated exact-match initialization loads the
  package-owned module, resolves its native-table bootstrap, and keeps the module loaded for the
  process lifetime; an owner constructor cannot infer function-pointer provenance independently.
...
- Package proof must show that the standalone analyzer package contains only compiler assets, a
  direct consumer receives its diagnostics, and no analyzer implementation enters runtime output
  or the Runtime package.
